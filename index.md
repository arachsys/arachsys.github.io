...
- [mailwrap](https://github.com/arachsys/mailwrap),
  [mailflow](https://github.com/arachsys/mailflow) and
  [mailflow2](https://github.com/arachsys/mailflow2) --- improved plain-text
  email in Apple Mail

  I sometimes work on an Apple machine running macOS, and it can be
  convenient to use the built-in IMAP mail client. It isn't too bad for
  ...
  messages, also fixing whitespace display bugs, and quoting/attribution
  in replies. MailWrap was an earlier plugin to add manual line-wrapping
  and paragraph filling shortcuts in the message composer. Such plugins are
  blocked on modern versions of macOS, as part of Apple's wider campaign
  against user freedom, though the bugs they fixed remain.

  MailFlow 2 is an alternative way to coax Apple Mail into emitting better
  plain-text messages. It registers as a MailKit message security handler,
  where 'encryption' strips the overzealous quoted-printable encoding and
  'signing' also flows the text RFC2646-style. Unlike the older plugins, it
  can't fix broken quoting/attribution or the display of leading whitespace.
...
- [pwgen](https://github.com/arachsys/pwgen) --- pronounceable passwords
  with reliable entropy

  Ted Ts'o's classic pwgen generates memorable passwords but with poor
  worst-case entropy: some outputs are hundreds of times more likely
  than others. This alternative implementation fixes that problem.
...
- [rledit](https://github.com/arachsys/rledit) -- use GNU Readline as an editor

  rledit is a command-line wrapper for GNU Readline to allow it to be used
  as a simple multi-line text editor, including vipe-style functionality.
...
- [ua](https://github.com/arachsys/ua) --- a simple command-line agent
  harness in Python

  This is a minimal agent harness script, providing a command-line
  interface to a large language model accessed through an Open Responses
  API, and allowing it to access local tools including a bash shell.
